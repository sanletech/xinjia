...
//查看完成的详细订单
    public function Order_Detail($order_num) {
        ...
        $listComment = Db::name('order_comment')->where('order_num',$order_num)->find(); 
        //查询对应的发票信息
        $listInvoice =Db::name('invoice')->where('id',$listComment['invoice'])->where('member_code'.$list['member_code'])->find();
