...
{   
    ...
    public function car_edit(){
          ...
          //获取需要修改的车队id
        $cs_id= $this->request->get('id'); 
       
         //根据修改的港口-车队ID获取对应的 港口 船公司信息
//         $sql="select c.id as car_id , c.car_name , p.id as port_id , p.port_name , p.city_id ,s.id as ship_id , s.ship_short_name "
//                 . "from hl_cardata c left join hl_car_ship_port csp on c.id =csp.car_data_id "
//                 . "left join hl_port p on csp.port_id=p.id left join hl_shipcompany s  on csp.ship_id=s.id  where(c.id='$id') order by port_id ";
        ...
        $sql="select CP.id cs_id, CP.port_id, P.port_name, CP.car_id ,CD.car_name, CD.address, CD.symbiosis, CD.status  ,"
               ...
               ."  where CP.id = '$cs_id'  group by CP.id  "  ;
