...
//            echo Db::getLastSql() ; 
    ...
    public function  lineCar($port_id,$address_data){
      ...
      //车运定价展示list
    public function price_transport_list($port_name ,$pages=15,$cl_id ='') {
        
         $list = Db::name('car_listprice')->alias('CP')
                ->join('hl_car_line L', 'CP.cl_id = L.id', 'left')
                ->join('hl_port P', 'L.port_id =P.port_code', 'left')
                ->field('CP.id ,CP.cl_id ,L.address_id ,L.address_name ,CP.car_id ,'
                        . 'L.port_id , P.port_name ,'
                        . 'CP.price_20GP ,CP.price_40HQ ,variable , '
                        . 'CP.latest_order_time')
                ->order('CP.cl_id, CP.variable')->buildSql();
        ...
        $lista =Db::table($list.' a')
                ->join("($list) as b",'b.cl_id =a.cl_id','left')
                ->field('a.cl_id ,a.port_id ,a.port_name,a.address_id ,a.address_name ,'
                        . 'a.id s_id,a.price_20GP s_20GP ,a.price_40HQ s_40HQ ,'
                        . 'a.latest_order_time s_ordertime , a.variable s_variable , '
                        .' b.id r_id ,b.price_20GP r_20GP ,b.price_40HQ r_40HQ '
                        . ',b.latest_order_time r_ordertime , b.variable r_variable ')
                ->where('a.variable', '<>','b.variable')
                ->group('a.cl_id, b.cl_id')
                ->paginate($pages,false,$pageParam);   
...
//            echo Db::getLastSql() ; 
    ...
    //车队运输定价 删除
    public function price_transport_del($carprice_id){
        $res = Db::name('car_listprice')->where('cl_id','in',$carprice_id )->delete();
        $res ?  $response['success'][] = '删除car_listprice表':$response['fail'][] = '删除car_listprice表';
        return $response ;
    }
    //车队运输定价的添加
    public function price_transport_toadd($port_id, $address_data, $load, $send){
        $cl_id = $this->lineCar($port_id,$address_data);
        $mtime =  time();
       // var_dump($load);exit;
        ...
        $send_price_40HQ = $send['price_40HQ'];
        //送货 s 装货 r
        $sql = "insert into hl_car_listprice(cl_id ,price_20GP,price_40HQ , variable ,mtime)  "
                . " values('$cl_id','$load_price_20GP','$load_price_40HQ','r','$mtime'),"
                . "       ('$cl_id','$send_price_20GP','$send_price_40HQ','s','$mtime')  ";
        $res = Db::execute($sql);
        $res ? $response['success'][] = '添加car_listprice表':$response['fail'][] = '添加car_listprice表';
        return $response ;
    }
    
    //车队运输定价的修改页面的原始数据
    public function  price_transport_edit($data) 
    {   
        $cl_id = $data['cl_id'];
        $port_name = '';
        $res = $this->price_transport_list($port_name ,$pages=5,$cl_id);
        
        return $res;
    }
    
    //车队运输定价的修改更新
    public function  price_transport_toedit($data)
    {   
       ...
       //装货和送货车队的id ,价格
        ...
        $sql = "update hl_car_listprice  set cl_id ='$cl_id',"
                . "price_20GP ='$load_price_20GP',price_40HQ ='$load_price_40HQ'"
                . ",mtime='$mtime'  where id ='$r_id'";
        $sql2 = "update hl_car_listprice  set cl_id ='$cl_id',"
                . "price_20GP ='$send_price_20GP',price_40HQ ='$send_price_40HQ'"
                . ",mtime='$mtime'  where id ='$s_id'";
        $res = Db::execute($sql); $res2 = Db::execute($sql2);
        $res ? $response['success'][] = '修改car_listprice表_装货':$response['fail'][] = '修改car_listprice表_装货';
        $res2 ? $response['success'][] = '修改car_listprice表_送货':$response['fail'][] = '修改car_listprice表_送货货';
        return $response;
    }
